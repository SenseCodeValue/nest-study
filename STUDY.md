...
### Test 소개
#### nest가 jest를 통해서 거의다 구성되해 놓았다.
    1. unit-test : unit-test는 격리된 function 하나 하나를 테스트하는것이다.
    2. e2e-test : 모든 시스템을 테스트하는것을 말한다. 
        - 사용자 관점에서 A 행위를 함에 있으메 필요한 기능들, 흐름을  모두 테스트한다.

### 간단히 jest 살펴보기
    - describe: 테스트를 묘사하다. 
    - befoceEach: 하나의 단위 아래의 모든 테스트가 실행되기전에 실행된다.  
